 ...
 * повторно. Но это безопасно: на уровне лида действует дневной guard по БД
 * ({@code BotExecutionReader#alreadySentToday}) — если лиду сегодня уже успешно отправляли
 * бота, повторный прогон его пропустит. Плюс повторной отправки одного и того же бота не
 * допускает {@code UNIQUE(lead_id, bot_id)}.
 */
@Slf4j
@Component
@AllArgsConstructor
