...
public enum BotRunType {
    ...
    /** Реализованные заказы розницы через N дней после закрытия: бот повторных продаж. */
    RETAIL_TO_REPEAT("Розница → повтор", "Реализованные заказы розницы через несколько дней после закрытия: бот повторных продаж");
